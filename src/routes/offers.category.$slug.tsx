      ...
      <main className="flex-1">
        {category && (
          <section className={`relative overflow-hidden bg-gradient-to-br ${category.color} py-16 text-white`}>
            <img src={category.cover} alt={category.nameAr} className="absolute inset-0 h-full w-full object-cover opacity-20" />
            <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
              ...
              <div className="mt-4 flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-3xl font-extrabold sm:text-4xl">{category.nameAr}</h1>
                  <p className="mt-1 text-sm text-white/80">{offers.length} عرض حصري متاح الآن</p>
                </div>
              </div>
            </div>
          </section>
        ...
        <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          {isLoading ? (
            ...
            <div className="rounded-2xl border border-dashed border-border bg-card p-12 text-center text-muted-foreground">
              لا توجد عروض حالياً في هذا التصنيف.
            </div>
          ) : (
