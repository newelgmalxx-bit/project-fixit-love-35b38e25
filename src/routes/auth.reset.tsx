...
function AuthResetPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch('https://koswmat.com/api/auth/reset-password', {
        method: 'POST',
        ...
        <div className="order-1 lg:order-2 flex items-center px-4 py-10 sm:px-12 lg:py-14">
          <div className="mx-auto w-full max-w-md">
            ...
            <form className="mt-6 w-full space-y-5" onSubmit={onSubmit}>
              <PwdField value={password} onChange={setPassword} label={lang === "ar" ? "كلمة المرور الجديدة" : "New password"} dir={dir} show={show} setShow={setShow} />
              <PwdField value={confirm} onChange={setConfirm} label={lang === "ar" ? "تأكيد كلمة المرور" : "Confirm password"} dir={dir} show={show} setShow={setShow} />
              <button
                type="submit"
