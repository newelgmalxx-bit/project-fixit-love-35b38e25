...
function CitiesPage() {
  ...
  async function load() {
    ...
    try {
      ...
      toast.error(e?.message || L("تعذّر تحميل المدن", "Failed to load cities"));
      setItems([]);
    } finally {
      setLoading(false);
    }
  ...
  async function save() {
    if (!editing.nameAr?.trim() || !editing.nameEn?.trim()) {
      toast.error(L("الاسم بالعربي والإنجليزي مطلوبان", "Arabic and English names are required"));
      return;
    }
    setSaving(true);
    try {
  ...
  async function toggle(c: AdminCity) {
    ...
    catch (e: any) { toast.error(e?.message || L("تعذّر التبديل", "Toggle failed")); }
  }

  async function remove(c: AdminCity) {
    const name = lang === "en" ? (c.nameEn || c.nameAr) : c.nameAr;
    if (!confirm(L(`حذف "${name}"؟`, `Delete "${name}"?`))) return;
    try { await adminCitiesApi.remove(c.id); toast.success(L("تم الحذف", "Deleted")); load(); }
  ...
  return (
    <AdminLayout
      title={L("المدن", "Cities")}
      subtitle={L(`${activeCount} مدينة مفعّلة من أصل ${items.length}`, `${activeCount} active out of ${items.length}`)}
      action={<PrimaryButton onClick={openNew}><Plus className="h-4 w-4" /> {L("مدينة جديدة", "New city")}</PrimaryButton>}
    >
      <PanelCard title="">
        <div className="mb-4 flex flex-wrap items-center gap-3">
          ...
          <button onClick={load} className="rounded-full border border-border px-4 py-1.5 text-xs font-bold">{L("تحديث", "Refresh")}</button>
        </div>

        {loading ? (
          <div className="flex justify-center py-16"><Loader2 className="h-6 w-6 animate-spin text-primary" /></div>
        ) : items.length === 0 ? (
          <div className="py-12 text-center text-sm text-muted-foreground">{L("لا توجد مدن.", "No cities.")}</div>
        ) : (
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md" dir={dir}>
          <DialogHeader><DialogTitle>{editing.id ? L("تعديل مدينة", "Edit city") : L("مدينة جديدة", "New city")}</DialogTitle></DialogHeader>
          <div className="grid gap-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-xs font-bold text-muted-foreground">{L("الترتيب", "Sort order")}</label>
