...
/** Parse "HH:MM" into total minutes. Returns null when invalid. */
...
/** Generate slots between open/close ("HH:MM"). If close <= open, returns []. */
export function generateTimeSlotsBetween(
  ...
  const interval = getSlotInterval(durationMinutes);
  const startMin = parseHHMM(open);
  const endMin = parseHHMM(close);
