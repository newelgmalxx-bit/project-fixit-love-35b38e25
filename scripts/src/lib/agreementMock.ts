...
export function buildAgreementHtmlForPartner(
  ...
  const seedNum = parseInt((p.id || "").replace(/\D/g, "").slice(-3) || "0", 10) || 1;
  const number = a
    ? `AGR-${new Date(a.created_at).getFullYear()}-${a.id.slice(-4).toUpperCase()}`
...
<title>عقد اشتراك - ${p.vendor_name}</title>
<style>
  ...
  <div class="inner">
    ...
    <div class="parties">
      ...
      <div class="party">
        ...
        <table>
          <tr><td class="k">اسم المركز</td><td class="v">${p.vendor_name || "—"}</td></tr>
          <tr><td class="k">المسؤول</td><td class="v">${p.owner_name || "—"}</td></tr>
          ${p.commercial_number ? `<tr><td class="k">السجل التجاري</td><td class="v">${p.commercial_number}</td></tr>` : ""}
          <tr><td class="k">الجوال</td><td class="v" dir="ltr">${p.phone || "—"}</td></tr>
          ${p.email ? `<tr><td class="k">البريد الإلكتروني</td><td class="v" dir="ltr">${p.email}</td></tr>` : ""}
          <tr><td class="k">المدينة</td><td class="v">${p.city || "—"}</td></tr>
        </table>
      </div>
    </div>
    ...
    <div class="signatures">
      ...
      <div class="sig">
        ...
        <div class="name">${a?.signed_name || p.owner_name || "—"}</div>
        <div class="role">${p.vendor_name || ""}</div>
      </div>
    </div>
