      ...
      <div className="flex flex-1 flex-col p-5">
        ...
        {offer.vendor?.name && (
          <div className="mt-2 flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <span className="truncate font-semibold text-foreground/80">{offer.vendor.name}</span>
          </div>
        )}

        {/* meta row */}
        <div className="mt-3 flex flex-wrap items-center gap-x-3 gap-y-1.5 text-xs">
          {offer.vendor?.rating > 0 && (
            <div className="flex items-center gap-1 rounded-full bg-amber-50 px-2 py-0.5 text-amber-700">
              <Star className="h-3.5 w-3.5 fill-amber-400 text-amber-400" />
              <span className="font-extrabold">{offer.vendor.rating}</span>
              <span className="text-amber-600/70">({offer.vendor.reviewsCount})</span>
            </div>
          )}
          {offer.vendor?.city && (
            <div className="flex items-center gap-1 text-muted-foreground">
              <MapPin className="h-3.5 w-3.5" />
              <span className="font-semibold">{offer.vendor.city}</span>
            </div>
          )}
