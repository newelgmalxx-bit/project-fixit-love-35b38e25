...
function pickDeposit(b: any): number | undefined {
  const v = b?.total_amount ?? b?.totalAmount ?? b?.total ?? b?.deposit_amount ?? b?.depositAmount ?? b?.deposit ?? b?.amountPaid ?? b?.amount_paid ?? b?.paidAmount ?? b?.paid_amount;
          ...
          <table className="w-full min-w-[1000px] text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="px-3 py-10 text-center text-xs text-muted-foreground">
                ...
                <tr key={b.id} className="border-b border-border hover:bg-muted/40">
                  ...
                  <td className="px-3 py-3 text-xs font-bold" dir="ltr">{pickDeposit(b) ?? "—"}</td>
          ...
          {viewing && (
            <div className="space-y-4 text-sm">
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <Info label={L("العربون", "Deposit")}><span dir="ltr">{pickDeposit(viewing) ?? "—"}</span></Info>
