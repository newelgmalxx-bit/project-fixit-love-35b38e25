...
function AdminDashboard() {
  ...
  async function loadPendingPartners() {
    let fromDb: any[] = [];
    try {
      const data = await adminPartnersApi.list({ status: "pending", limit: 20 });
      fromDb = (data.items || []).map((p: any) => ({
        id: p.id,
  ...
  useEffect(() => {
    adminApi.analytics()
      .then((d: any) => {
        ...
        setStats((s: any) => ({
          ...s,
          ...data,
          // Backend now returns commission as revenue (إيراد المنصة)
          revenue: Number(data.totalRevenue ?? data.revenue ?? s.revenue) || 0,
          monthRevenue: Number(data.monthRevenue ?? data.totalRevenue ?? 0) || 0,
          totalBookingsValue: Number(data.totalBookingsValue ?? 0) || 0,
          totalPartnerEarnings: Number(data.totalPartnerEarnings ?? 0) || 0,
          totalBookings: Number(data.totalBookings ?? s.totalBookings ?? 0) || 0,
          ordersCount: Number(data.ordersCount ?? data.totalBookings ?? s.ordersCount ?? 0) || 0,
          avgOrderValue: Number(data.avgOrderValue ?? s.avgOrderValue ?? 0) || 0,
          revenueGrowth: Number(data.revenueGrowth ?? data.growthRate ?? s.revenueGrowth ?? 0) || 0,
        }));
        ...
        if (Array.isArray(data.salesByCategory) && data.salesByCategory.length) {
          const palette = ["#00AEC6", "#3a7fbe", "#5fa1d9", "#9bc4e8", "#cbe0f0"];
          setByCat(data.salesByCategory.map((s: any, i: number) => ({ ...s, color: palette[i % palette.length] })));
        }
      })
    ...
    adminApi.orders.list({ limit: 200 })
      .then((p: any) => {
        ...
        // Backend now provides revenue/aggregates; only derive UI fallbacks here.
        ...
        setStats((s: any) => ({
          ...s,
          // Don't override revenue/avgOrderValue from analytics; only fill if missing
          ordersCount: s.ordersCount || all.length,
          totalBookings: s.totalBookings || all.length,
